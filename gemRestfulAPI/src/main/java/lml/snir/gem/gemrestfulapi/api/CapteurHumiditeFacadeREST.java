...
public class CapteurHumiditeFacadeREST extends AbstractFacade<CapteurHumidite> {
    ...
    @Path("{id}")
    @Consumes({MediaType.APPLICATION_XML, MediaType.APPLICATION_JSON})
    public void edit(@PathParam("id") Long id, CapteurHumidite entity) {
        super.edit(entity);
    }
    ...
    public CapteurHumidite find(@PathParam("id") Long id) {
    ...
    @Path("{from}/{to}")
    @Produces({MediaType.APPLICATION_XML, MediaType.APPLICATION_JSON})
    public List<CapteurHumidite> findRange(@PathParam("from") Integer from, @PathParam("to") Integer to) {
