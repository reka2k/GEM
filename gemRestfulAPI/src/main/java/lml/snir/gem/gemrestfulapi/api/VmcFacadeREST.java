...
public class VmcFacadeREST extends AbstractFacade<Vmc> {
    ...
    @Path("{id}")
    @Consumes({MediaType.APPLICATION_XML, MediaType.APPLICATION_JSON})
    public void edit(@PathParam("id") Long id, Vmc entity) {
        super.edit(entity);
    }
    ...
    public Vmc find(@PathParam("id") Long id) {
    ...
    @Path("{from}/{to}")
    @Produces({MediaType.APPLICATION_XML, MediaType.APPLICATION_JSON})
    public List<Vmc> findRange(@PathParam("from") Integer from, @PathParam("to") Integer to) {
