...
public class ChauffageFacadeREST extends AbstractFacade<Chauffage> {
    ...
    @Path("{id}")
    @Consumes({MediaType.APPLICATION_XML, MediaType.APPLICATION_JSON})
    public void edit(@PathParam("id") Long id, Chauffage entity) {
        super.edit(entity);
    }
    ...
    public Chauffage find(@PathParam("id") Long id) {
    ...
    @Path("{from}/{to}")
    @Produces({MediaType.APPLICATION_XML, MediaType.APPLICATION_JSON})
    public List<Chauffage> findRange(@PathParam("from") Integer from, @PathParam("to") Integer to) {
