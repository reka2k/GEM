...
public class CompteurFacadeREST extends AbstractFacade<Compteur> {
    ...
    @Path("{id}")
    @Consumes({MediaType.APPLICATION_XML, MediaType.APPLICATION_JSON})
    public void edit(@PathParam("id") Long id, Compteur entity) {
        super.edit(entity);
    }
    ...
    public Compteur find(@PathParam("id") Long id) {
    ...
    @Path("{from}/{to}")
    @Produces({MediaType.APPLICATION_XML, MediaType.APPLICATION_JSON})
    public List<Compteur> findRange(@PathParam("from") Integer from, @PathParam("to") Integer to) {
