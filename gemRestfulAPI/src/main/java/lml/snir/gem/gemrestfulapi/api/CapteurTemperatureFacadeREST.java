...
public class CapteurTemperatureFacadeREST extends AbstractFacade<CapteurTemperature> {
    ...
    @Path("{id}")
    @Consumes({MediaType.APPLICATION_XML, MediaType.APPLICATION_JSON})
    public void edit(@PathParam("id") Long id, CapteurTemperature entity) {
        super.edit(entity);
    }
    ...
    public CapteurTemperature find(@PathParam("id") Long id) {
    ...
    @Path("{from}/{to}")
    @Produces({MediaType.APPLICATION_XML, MediaType.APPLICATION_JSON})
    public List<CapteurTemperature> findRange(@PathParam("from") Integer from, @PathParam("to") Integer to) {
